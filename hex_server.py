...
# MODIFIED VERSION BY MANUEL IGLESIAS TO INTERFACE WITH 
...
Returns: 
	The size of data. The address of data. The type of data. 
	The line checksum. 
...
Parameters:
	...
	timeout: The maximum time in milliseconds the function 
	can be running before a time out.
Returns: 
...
Returns: 
	Nothing
Description: 
	It starts the STK500 protocol to program the data 
	at their respective memory address.
'''
...
#-------------------------------------------------------------------------------------------------		
def main():
	print "Arduino Hex Server - Listen to connections"
		...
		while True:
			...
			for s in rlist:
				if s == ser:
					...
					'''
					It assures the connection is for programming an Arduino 
					and not other service. '''
					if wait_for(cli, "hello", 5000)[0]:
						...
						if ok:
							...
							if read_hex_file(chunks, received.strip()):
								cli.send("ok")
								if wait_for(cli, "ready", 2*MAX_TIMEOUT)[0]:
									print "send programming data"
